...
const GENERATED_DISPLAY_NAME_SET = new Set(
    DISPLAY_NAME_ADJECTIVES.flatMap((adjective) =>
        DISPLAY_NAME_NOUNS.map((noun) => formatDisplayName(adjective, noun)),
    ),
)

/**
 * Returns true when a name matches the app's autogenerated adjective+noun pool.
 * This is used to separate system-assigned defaults from user-customized names.
 */
export function isSystemGeneratedDisplayName(name: string | null | undefined): boolean {
    if (!name) {
        return false
    }

    return GENERATED_DISPLAY_NAME_SET.has(normalizeDisplayName(name))
}

export function isCustomDisplayName(name: string | null | undefined): boolean {
    if (!name) {
        return false
    }

    return !isSystemGeneratedDisplayName(name)
}
