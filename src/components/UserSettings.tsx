...
export default function UserSettings({ 
    ...
    // Helper to normalize display name (trim and collapse spaces)
    const normalizeDisplayName = (name: string): string => {
        return name.trim().replace(/\s+/g, ' ');
    };

    // Helper to check if display name has changed (case-insensitive, trimmed)
    const hasDisplayNameChanged = (newName: string, currentName: string | null | undefined): boolean => {
        const normalizedNew = normalizeDisplayName(newName);
        const normalizedCurrent = currentName ? normalizeDisplayName(currentName) : '';
        return normalizedNew.toLowerCase() !== normalizedCurrent.toLowerCase();
    };
                                ...
                                <div className="flex gap-2">
                                    <div className="flex-1">
                                        <input
                                            ...
                                            onChange={(e) => {
                                                setLocalDisplayName(e.target.value.slice(0, 20))
                                        ...
                                        {!displayNameError && (
                                            ...
                                            >
                                                3–20 characters; letters, numbers, spaces, and: . _ - (case-insensitive, unique)
                                            </p>
                                        )}
                                        {displayNameError && (
                                            <p 
                                        ...
                                        {!displayNameError && localDisplayName.length > 0 && normalizeDisplayName(localDisplayName).length < 3 && (
                                            <p 
                                                id="display-name-error" 
                                                className="mt-1 text-xs text-red-600"
                                            >
                                                Must be at least 3 characters (after trimming spaces)
                                            </p>
                                        )}
                                    </div>
                                    <button
                                        ...
                                        disabled={
                                            ...
                                            normalizeDisplayName(localDisplayName).length < 3 || 
                                            normalizeDisplayName(localDisplayName).length > 20 || 
                                            !hasDisplayNameChanged(localDisplayName, displayName)
                                        }
                                        className="
