...
export default function GameBoardById() {
    ...
    // Initialize game from server - runs only once
    useEffect(() => {
        ...
        const initGame = async () => {
            ...
            try {
                ...
                // Restore answered questions from game state
                ...
                if (round === 'FINAL') {
                    // Game is in Final Jeopardy
                    setCurrentRound('DOUBLE')
                    setIsDoubleJeopardy(true)
                    
                    // Check if Final Jeopardy question was already answered
                    ...
                    if (fjAnswered && fjQuestionId) {
                        // Final Jeopardy was already answered - show result screen
                        const fjResult = gameData.questions[fjQuestionId]
                        
                        // Load the Final Jeopardy question to display
                        try {
                            const params = new URLSearchParams()
                            params.append('questionId', fjQuestionId)
                            params.append('mode', config.mode || 'random')
                            params.append('gameId', gameId) // Pass gameId for spoiler policy
                            ...
                            if (response.ok) {
                                ...
                                // Handle case where the question violates spoiler policy
                                const errorData = await response.json().catch(() => ({}))
                                setError(errorData.error || 'Failed to load Final Jeopardy')
                            }
                        } catch (error) {
                            console.error('Error loading Final Jeopardy result:', error)
                        }
                        setLoading(false)
                    } else if (fjQuestionId) {
                        // Final Jeopardy started but not answered - restore state
                        try {
                            const params = new URLSearchParams()
                            params.append('questionId', fjQuestionId)
                            params.append('mode', config.mode || 'random')
                        ...
                        // Final Jeopardy not yet started - load it fresh
                        setLoading(false)
